...
export class AdminProductsService {
  ...
  async findAll(query: AdminProductQueryDto) {
    ...
    return {
      data: products.map((product) => {
        ...
        const lessonsByType = allLessons.reduce(
          (acc, lesson) => {
  ...
  async getStats() {
    ...
    const [total, published, draft, archived, withEnrollments] =
      await Promise.all([
        ...
        prisma.product.count({
          where: { deleted_at: null, status: 'archived' },
        }),
        prisma.product.count({
          where: {
            deleted_at: null,
                            ...
                            create: m.lessons.map((l) => ({
                              ...
                              type:
                                typeof l.type === 'string'
                                  ? LessonType[
                                      l.type as keyof typeof LessonType
                                    ]
                                  : l.type,
                              ...
                              status: l.status
                                ? typeof l.status === 'string'
                                  ? LessonStatus[
                                      l.status as keyof typeof LessonStatus
                                    ]
                                  : l.status
                                : LessonStatus.draft,
                              order: l.order ?? 0,
                              ...
                              question:
                                l.quiz_questions && l.quiz_questions.length > 0
                                  ? {
  ...
  async update(id: string, data: Partial<CreateProductDto>) {
    ...
    if (modules && Array.isArray(modules)) {
      ...
      // Xóa user lesson score
      await prisma.userLessonScore.deleteMany({
        where: {
          lesson: {
            module: {
              course_id: id,
